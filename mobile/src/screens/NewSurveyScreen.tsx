...
export const NewSurveyScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const handleCreate = async () => {
    ...
    console.log("Creating survey for real user:", userId);
    ...
    try {
      ...
      console.log("Create survey response status:", res.status);
      
      if (!res.ok) {
        const text = await res.text();
        console.error("Create survey error:", text);
        throw new Error(text);
      }

      const data = await res.json();
      console.log("Real survey created:", data);
