# 🔧 Full Server Setup - Everything Included

## 🚨 Проблема: docker-compose: command not found

### ✅ РЕШЕНИЕ - Полная установка одной командой:

```bash
# Подключитесь к серверу
ssh root@your-server-ip

# Выполните ПОЛНУЮ установку (включает Docker, Docker Compose, и приложение):
cd /opt && rm -rf industrial-inspector && apt update && apt install -y git curl wget && curl -fsSL https://get.docker.com -o get-docker.sh && sh get-docker.sh && usermod -aG docker root && curl -L "https://github.com/docker/compose/releases/download/v2.20.0/docker-compose-$(uname -s)-$(uname -m)" -o /usr/local/bin/docker-compose && chmod +x /usr/local/bin/docker-compose && systemctl restart docker && git clone https://github.com/lacrenol/industrial-inspector.git && cd industrial-inspector && mkdir -p production && cat > production/docker-compose.vertex.yml << 'COMPOSE_EOF'
version: "3.8"
services:
  backend:
    build: 
...
volumes:
  redis_data:
COMPOSE_EOF
cat > production/Dockerfile.vertex << 'DOCKERFILE_EOF'
...
Pillow==10.4.0
REQ_EOF
cat > production/update-server.sh << 'UPDATE_EOF'
#!/bin/bash
set -e
...
cd production
/usr/local/bin/docker-compose -f docker-compose.vertex.yml down
/usr/local/bin/docker-compose -f docker-compose.vertex.yml build --no-cache
/usr/local/bin/docker-compose -f docker-compose.vertex.yml up -d
sleep 30
echo "🏥 Health check..."
if curl -f http://localhost/api/health > /dev/null 2>&1; then
    ...
    /usr/local/bin/docker-compose -f docker-compose.vertex.yml logs backend
    exit 1
fi
UPDATE_EOF
...
### 📦 Устанавливает ВСЕ необходимое:
1. **apt update** - обновляет систему
2. **git curl wget** - базовые утилиты
3. **Docker** - устанавливает Docker Engine
4. **Docker Compose** - устанавливает Docker Compose
5. **systemctl restart docker** - перезапускает Docker
6. **Клонирует репозиторий** - скачивает код
7. **Создает ВСЕ файлы** - Docker, backend, nginx
8. **Запускает приложение** - развертывает все контейнеры

### 🔧 Исправляет проблему docker-compose:
- Использует полный путь: `/usr/local/bin/docker-compose`
- Устанавливает Docker Compose вручную
...
Теперь update скрипт будет работать:
```bash
cd /opt/industrial-inspector
...
**КОПИРУЙТЕ ОДНУ КОМАНДУ ВЫШЕ И ВЫПОЛНЯЙТЕ!**
...
- ✅ API доступен
- ✅ Update скрипт работает

**Больше никаких проблем с docker-compose!** 🚀
